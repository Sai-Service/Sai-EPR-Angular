        ...
        (data => {
          ...
          for (let i = 0; i < this.trfLinesList().length; i++) {
            ...
            this.locData[i]={locatorId:this.lstcomment[i].locatorId,
              segmentName:this.lstcomment[i].segmentName,
              onHandQty:this.lstcomment[i].primaryQty,
              id:this.lstcomment[i].locatorId}
              let selLocator = this.locData.find(d => d.locatorId === this.lstcomment[i].locatorId);
             
            trxLnArr1.controls[i].patchValue({ locatorId: selLocator.locatorId });
             trxLnArr1.controls[i].patchValue({segmentName: selLocator.segmentName});
            trxLnArr1.controls[i].patchValue({ onHandQty: this.lstcomment[i].primaryQty });
            trxLnArr1.controls[i].patchValue({ onHandId: this.lstcomment[i].id });
          }
