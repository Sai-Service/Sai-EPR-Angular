...
export class SaiExtendedWarrantyComponent implements OnInit {
  ...
  receiptDetailsList: Array<string> = [];
  ...
  onEnterKms(x: any) {
    ...
    if (this.lastRunKms === null) {
      this.lastRunKms = 0;
      // alert("Last Run Km :" + this.lastRunKms);
    }

    if (mKms > 0 && this.variant != null && mKms >= this.lastRunKms) { this.LoadEWSchemeVariant(mVAriant, mAging, mKms); }
    else {
  ...
  receiptDetailsNew() {
    var orderNumber = this.saiEwForm.get('orderNumber')?.value;
    this.service.receiptDetailsNewFn(orderNumber).subscribe((res: any) => {
      if (res.code === 200) {
        // alert(res.message);
        this.receiptDetailsList = res.obj;
      } else {
        if (res.code === 400) {
          alert(res.message);
          window.location.reload();
