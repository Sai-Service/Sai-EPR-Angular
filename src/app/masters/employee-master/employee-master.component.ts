...
onke(evert:any){
  var divId=this.employeesMasterForm.get('divisionId')?.value;
  alert(divId)
}
...
convUpperCase(){
  const formValue = (this.employeesMasterForm.value);
...
newMast() {
  ...
  const formValue= this.transData(this.employeesMasterForm.value);
...
updateMast() {
  ...
  let select = this.lstcomments.find((d:any) => d.divisionName === this.divisionName);
  console.log(select);
...
Select(emplId: number) {
  ...
  let select = this.lstcomments.find((d:any) => d.emplId === emplId);
  if (select) {
...
onOptionsDEPTSelected(event:any){ 
  var dept =this.employeesMasterForm.get('deptId')?.value;
  alert(dept)
  let select = this.DepartmentList.find((d:any) => d.cmnTypeId === dept);
  alert(select.code)
...
CheckDataValidations() {
 ...
 if( formValue.loginAccess==='Y' ) {
  ...
  validateDob(event:any) {
    var dob=this.employeesMasterForm.get('dob')?.value;
    alert(dob)
    var currDate = new Date();
    alert(currDate);
    var sDate = new Date(dob);
    if (sDate >= currDate) {
