...
@Component({
  selector: 'app-pump-misc-transaction',
  templateUrl: './pump-misc-transaction.component.html',
  styleUrls: ['./pump-misc-transaction.component.css']
})
export class PumpMiscTransactionComponent implements OnInit {
  pumpMiscellaneousForm: FormGroup;
  ...
  constructor(
    ...
    this.pumpMiscellaneousForm = fb.group({
      compNo: [''],
      compileName: [''],
      compileId: [''],
      locId: [''],
  ...
  cycleLinesList(): FormArray {
    return this.pumpMiscellaneousForm.get('cycleLinesList') as FormArray;
  }
  ...
  addnewcycleLinesList(i: number) {
    // alert(i);
    if (i > -1) {
      var trxLnArr1 = this.pumpMiscellaneousForm.get('cycleLinesList').value;
    ...
    //alert('hi');
    // alert(this.pumpMiscellaneousForm.get('compileType').value+'value');
    // this.cycleLinesList().push(this.newcycleLinesList());
    // =======================================================

          // if (i > -1 && this.pumpMiscellaneousForm.get('compileType').value === 4) {
    ...
    if (len1 == i + 1) {
      ...
      if (document.contains(btnrm)) {
        (document.getElementById('btnrm' + i) as HTMLInputElement).disabled =
          false;
        this.pumpMiscellaneousForm.get('compileType').disable();
        this.pumpMiscellaneousForm.get('reason').disable();
        this.pumpMiscellaneousForm.get('approvedBy').disable();
        // this.pumpMiscellaneousForm.get('cycleLinesList').get('LocatorSegment').disable();
  ...
  removenewcycleLinesList(trxLineIndex) {
    ...
    // this.cycleLinesList().removeAt(trxLineIndex);
    var formVal = this.pumpMiscellaneousForm.get('cycleLinesList').value;
    var patch = this.pumpMiscellaneousForm.get('cycleLinesList') as FormArray;
    var len = this.cycleLinesList().length;
  ...
  ngOnInit(): void {
    ...
    this.service.TransactionTypemisc().subscribe((data) => {
      ...
      if (this.dispheader === true) {
        // this.pumpMiscellaneousForm.reset();
    ...
    this.service.ReasonList().subscribe((data) => {
      ...
      for (let i = 0; i < this.reasonlist.length; i++) {
        if (this.reasonlist[i].reasonName.includes('SAPP')) {
    ...
    //   var patch = this.pumpMiscellaneousForm.get('trxLinesList') as  FormArray
  ...
  pumpMiscellaneous(pumpMiscellaneousForm: any) {}
  ...
  openCodeCombination() {
    let SegmentName1 = this.pumpMiscellaneousForm.get('SegmentName').value;
    if (SegmentName1 === null) {
      this.pumpMiscellaneousForm.get('segment11').reset();
      this.pumpMiscellaneousForm.get('segment2').reset();
      this.pumpMiscellaneousForm.get('segment3').reset();
      this.pumpMiscellaneousForm.get('segment4').reset();
      this.pumpMiscellaneousForm.get('segment5').reset();

      this.pumpMiscellaneousForm.get('lookupValueDesc1').reset();
      this.pumpMiscellaneousForm.get('lookupValueDesc2').reset();
      this.pumpMiscellaneousForm.get('lookupValueDesc3').reset();
      this.pumpMiscellaneousForm.get('lookupValueDesc4').reset();
      this.pumpMiscellaneousForm.get('lookupValueDesc5').reset();
    }
    if (SegmentName1 != null) {
  ...
  fnCancatination() {
    this.segmentName =
      this.pumpMiscellaneousForm.get('segment11').value +
      '.' +
      this.pumpMiscellaneousForm.get('segment2').value +
      '.' +
      this.pumpMiscellaneousForm.get('segment3').value +
      '.' +
      this.pumpMiscellaneousForm.get('segment4').value +
      '.' +
      this.pumpMiscellaneousForm.get('segment5').value;
    ...
    this.service.segmentNameList(this.segmentName).subscribe((data) => {
      ...
      if (this.segmentNameList.code === 200) {
        this.pumpMiscellaneousForm.patchValue({
        ...
        this.pumpMiscellaneousForm.patchValue({ segmentName: '' });
  ...
  reservePos(i) {
    //alert("Hello");
    var trxLnArr1 = this.pumpMiscellaneousForm.get('cycleLinesList').value;
    const formValue: Imiscellaneous = this.pumpMiscellaneousForm.value;
    let variants = <FormArray>this.cycleLinesList();
    var transtypeid = this.pumpMiscellaneousForm.get('compileType').value;
    var seltranstyp = this.transType.find(
      (d) => d.transactionTypeId === transtypeid
    );
    var locId1 = this.pumpMiscellaneousForm.get('locId').value;
  ...
  validate(i: number, qty1) {
    ...
    if (
      qty > avalqty &&
      this.pumpMiscellaneousForm.get('compileType').value !== 13
    ) {
  ...
  searchByCompileID(itemId) {
    // alert(itemId+'ID')
    var compileId = this.pumpMiscellaneousForm.get('compileId').value;
    ...
    this.service.getsearchByCompId(compileId, itemId).subscribe((data) => {
      ...
      if (data.code === 200) {
        ...
        console.log(xx);
        let patch = this.pumpMiscellaneousForm.get('cycleLinesList') as FormArray;
        var control = this.pumpMiscellaneousForm.get('cycleLinesList').value;
        var len = this.cycleLinesList().length;
        ...
        // this.pumpMiscellaneousForm.get('cycleLinesList').patchValue(data.obj);
        //  for(let i=0; i< data.obj.length; i++){
        //   //   alert(data.obj.cycleLinesList[i].subInventory+'subInventory');
        // this.pumpMiscellaneousForm.patchValue({'srlNo':i+1})
        // control.controls[i].patchValue({srlNo:i+1  })

        // this.pumpMiscellaneousForm.patchValue({'segment':data.obj.segment});
        // this.pumpMiscellaneousForm.patchValue({'subInventory':data.obj[i].subInventory});
        // this.pumpMiscellaneousForm.patchValue({'compileLineId':data.obj[i].compileLineId})
        // }
      }
    });
  }
  search(compno) {
    ...
    if (compno != undefined) {
      ...
      // var compno = this.pumpMiscellaneousForm.get('compNo').value;
      var appflag = this.pumpMiscellaneousForm.get('trans').value;
      // alert(compno);
      ...
      this.service.getSearchViewBycompNo(compno).subscribe((data) => {
        ...
        if (data.code === 200) {
          ...
          let control = this.pumpMiscellaneousForm.get(
            'cycleLinesList'
          ) as FormArray;
          var len = this.cycleLinesList().length;
          ...
          this.pumpMiscellaneousForm.patchValue(data.obj);
          this.currentOp = 'INSERT';
          // this.pumpMiscellaneousForm.get('cycleLinesList').patchValue(data.obj.cycleLinesList);
          this.pumpMiscellaneousForm.disable();
          // this.dispRow=false;
          this.displayaddButton = false;
          this.displayButton = false;
          this.displayOp=false;
          // this.pumpMiscellaneousForm.get('cycleLinesList').disable();
        }
      });
    }
  }
  ...
  onSelectReason(event) {
    // alert(event);
    // var reasname=this.pumpMiscellaneousForm.get('reason').value;
    ...
    this.service
      ...
      .subscribe((data) => {
        ...
        // this.pumpMiscellaneousForm.patchValue({reason:this.acccodedesc.segmentName});
  ...
  HeaderValidation() {
    ...
    Object.keys(this.pumpMiscellaneousForm.controls).forEach((key) => {
      const control = this.pumpMiscellaneousForm.controls[key] as
        | FormControl
        | FormArray
        | FormGroup;
      ...
      if (control instanceof FormControl) {
        ...
        (<FormArray>(
          this.pumpMiscellaneousForm.get('cycleLinesList')
  ...
  getGroupControl(fieldName) {
    return this.pumpMiscellaneousForm.get(fieldName);
  }

  getGroupControllinewise(index, fieldName) {
    // alert('nam'+fieldName);
    return (<FormArray>this.pumpMiscellaneousForm.get('cycleLinesList'))
      .at(index)
      .get(fieldName);
  }
  deleteReserve() {
  ...
  viewMiscnote() {
    var shipNumber = this.pumpMiscellaneousForm.get('compileName').value;
