...
@Component({
  selector: 'app-sales-order-proforma-chetak',
  templateUrl: './sales-order-proforma-chetak.component.html',
  styleUrls: ['./sales-order-proforma-chetak.component.css']
})
export class SalesOrderProformaChetakComponent implements OnInit {
  salesOrderBookingChetakForm: FormGroup;
  ...
  custRefAddr:string;
  // custPoDate: Date;
  custPoDate = this.pipe.transform(Date.now(), 'y-MM-dd');
  state: string;
  customerId: number;
  ...
  constructor(private fb: FormBuilder, private location1: Location, private service: MasterService, private orderManagementService: OrderManagementService) {
    this.salesOrderBookingChetakForm = fb.group({
      orderNumber: [],
      emailId: [],
      classCodeType: [],
      emailId1: [],
  ...
  orderlineDetailsArray(): FormArray {
    return <FormArray>this.salesOrderBookingChetakForm.get('oeOrderLinesAllList')
  }
  ...
  salesOrderBookingChetak(salesOrderBookingChetakForm: any) {
  }

  get f() { return this.salesOrderBookingChetakForm.controls }
  ...
  ngOnInit(): void {
      ...
      .subscribe(
        data => {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            //   if (data[i].ouId === 999) {
                this.salesOrderBookingChetakForm.patchValue({ priceListName: data[i].priceListName })
                this.salesOrderBookingChetakForm.patchValue({ priceListId: data[i].priceListHeaderId })
            //   }
            // }
    ...
    var patch = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray
  ...
  onOptionsSelectedVariant(mainModel) {
    var issuedBy = this.salesOrderBookingChetakForm.get('issuedBy').value;
    // alert(issuedBy);
  ...
  onOptionsSelectedBasicPrice(color: any) {
    ...
    if (Number(sessionStorage.getItem('divisionId')) === 2) {
      var model = this.salesOrderBookingChetakForm.get('model').value;
      var variant = this.salesOrderBookingChetakForm.get('variant').value;
      this.orderManagementService.dealerShipBaseAmtNew(model, variant, color,sessionStorage.getItem('ouId'))
        .subscribe(
          data => {
            this.salesOrderBookingChetakForm.patchValue({ basicValue: data.obj[0].basicValue })
            this.basicChassisPrice = data.obj[0].basicValue

            var patch = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
            this.displaysegmentInvType[0] = false;
            var chassisItem = this.salesOrderBookingChetakForm.get('model').value + '-' +
              this.salesOrderBookingChetakForm.get('variant').value + '-' +
              this.salesOrderBookingChetakForm.get('color').value;
      ...
      .subscribe(
        data => {
          if (data.code === 200) {
            ...
            let orLineCtrl = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
            // debugger;
            ...
            this.salesOrderBookingChetakForm.patchValue(data.obj);
            this.salesOrderBookingChetakForm.patchValue({ name: data.obj.billLocName });
      ...
      .subscribe(
        data => {
          if (data.code === 200) {
            ...
            this.salesOrderBookingChetakForm.patchValue({ custAccountNo: custAccountNo });
            let select = this.payTermDescList.find(d => d.lookupValueId === this.selCustomer.termId);
            // this.paymentType = select.lookupValue;
            this.salesOrderBookingChetakForm.patchValue({ paymentType: select.lookupValue })
            this.salesOrderBookingChetakForm.get('custName').disable();
            this.salesOrderBookingChetakForm.get('mobile1').disable();
            // alert(this.custSiteList.length)
            for (let i = 0; i < this.custSiteList.length; i++) {
              if (this.custSiteList.length === 1) {
                // alert(this.custSiteList.length)
                this.salesOrderBookingChetakForm.patchValue({ name: this.custSiteList[0].siteName });
            ...
            this.salesOrderBookingChetakForm.get('custAccountNo').disable();
      ...
      .subscribe(
        data => {
          if (data.code === 200) {
            ...
            this.salesOrderBookingChetakForm.patchValue({ salesRepId: data.obj.emplId });
            this.salesOrderBookingChetakForm.patchValue({ salesRepName: salesExicustive })
          }
        })
  }
  ...
  onOptionsSelectedcustSiteName(siteName) {
    ...
    if (selSite.ouId != (sessionStorage.getItem('ouId'))) {
      ...
      this.salesOrderBookingChetakForm.patchValue(selSite);
      ...
      if (selSite.disPer != null) {
        ...
        this.salesOrderBookingChetakForm.patchValue({ disPer: selSite.disPer });
        this.salesOrderBookingChetakForm.patchValue({ custtaxCategoryName: selSite.taxCategoryName })
    ...
    this.service.searchCustomerByContact(contactNo)
      .subscribe(
        data => {
          ...
          this.salesOrderBookingChetakForm.patchValue({ custAccountNo: data.obj.custAccountNo })
  ...
  panCardWalding(event: any) {
    this.orderManagementService.searchByPanNumber(this.salesOrderBookingChetakForm.get('walkCustPan').value)
      .subscribe(
        data => {
          if (data.code === 200) {
            if (data.obj.length >= 0) {
              alert(data.message + ' ' + 'Customer Account Number' + ' ' + data.obj.accountNo)
              this.salesOrderBookingChetakForm.get('walkCustPan').reset();
            }
  ...
  walkcustomermerge(event: any) {
    var contactName = this.salesOrderBookingChetakForm.get('walkCustName').value + '#' + this.salesOrderBookingChetakForm.get('walkCustPan').value + '#' + this.salesOrderBookingChetakForm.get('walkCustaddres').value
    this.salesOrderBookingChetakForm.patchValue({ cntrOrdCustName: contactName })
  }

  onOptionsSelectedissueTypeCode(event: any) {
    let selectIssueCode = this.issueCodeTypeList.find(d => d.codeDesc === event);
    this.salesOrderBookingChetakForm.patchValue({ issueCode: selectIssueCode.code })
    var concatissuetypecode = this.salesOrderBookingChetakForm.get('issueCode').value + '-' + this.salesOrderBookingChetakForm.get('issueCodeType1').value
    this.salesOrderBookingChetakForm.patchValue({ issueCodeType: concatissuetypecode });
  }
  ...
  onOptionsSelectedCategory(orderType: string, lnNo: number) {
    ...
    let controlInv2 = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
    let controlinv1 = controlInv2.getRawValue();
    let controlinv = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
    if (controlinv1[lnNo].invType != 'SS_VEHICLE') {
      if (this.salesOrderBookingChetakForm.get('model').value === undefined || this.salesOrderBookingChetakForm.get('model').value == null ||
        this.salesOrderBookingChetakForm.get('variant').value === undefined || this.salesOrderBookingChetakForm.get('variant').value == null ||
        this.salesOrderBookingChetakForm.get('fuelType').value === undefined || this.salesOrderBookingChetakForm.get('fuelType').value == null ||
        this.salesOrderBookingChetakForm.get('color').value === undefined || this.salesOrderBookingChetakForm.get('color').value == null ||
        this.salesOrderBookingChetakForm.get('basicValue').value === undefined || this.salesOrderBookingChetakForm.get('basicValue').value == null
      ) {
        alert('Select Vehicle Details & then add line item..!');
        return;
      }
      if (this.salesOrderBookingChetakForm.get('custAccountNo').value === undefined || this.salesOrderBookingChetakForm.get('custAccountNo').value === null ||
        this.salesOrderBookingChetakForm.get('name').value === undefined || this.salesOrderBookingChetakForm.get('name').value === null) {
  ...
  onOptionsSelectedDescription(segment: string, k) {
    ...
    if (itemType != 'SS_VEHICLE') {
      ...
      console.log(select);
      this.salesOrderBookingChetakForm.patchValue({ itemId: select.itemId })
      this.itemId = select.itemId;
      var custtaxCategoryName = this.salesOrderBookingChetakForm.get('taxCategoryName').value;
      var priceListId = this.salesOrderBookingChetakForm.get('priceListId').value;
          ...
          .subscribe(
            data => {
              if (data.code === 200) {
                ...
                let controForPrice = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList').value;
  ...
  onKey(index, fldName) {
    ...
    var trxLnArr3 = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
    ...
    var arrayControlNew = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
      ...
      this.service.taxCalforItem(itemId, taxCategoryId, disAmt1, baseAmtAfterDisc)
        .subscribe(
          (data: any[]) => {
            ...
            let distAmtArray = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
  ...
  addRow(i) {
    // alert(i)
    var trxLnArr1 = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList').value;
    var trxLnArr2 = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
    ...
    var disPer = this.salesOrderBookingChetakForm.get('disPer').value;
  ...
  updateTotAmtPerline(lineIndex) {
    var formArr = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
    ...
    this.salesOrderBookingChetakForm.patchValue({ 'subtotal': basicAmt });
    disAmt = Math.round(((disAmt) + Number.EPSILON) * 100) / 100;
    this.salesOrderBookingChetakForm.patchValue({ 'discAmt': disAmt });
    taxAmt1 = Math.round(((taxAmt1) + Number.EPSILON) * 100) / 100;
    this.salesOrderBookingChetakForm.patchValue({ 'totTax': taxAmt1 });
    totAmt = Math.round(((totAmt) + Number.EPSILON) * 100) / 100;
    this.salesOrderBookingChetakForm.patchValue({ 'totAmt': totAmt });
    this.salesOrderBookingChetakForm.patchValue({ 'tcsAmt': tcsAmt1 });
  ...
  createProformaOrder() {
    ...
    jsonData.refCustNo = this.salesOrderBookingChetakForm.get('refCustNo').value;
    jsonData.custPoNumber = this.salesOrderBookingChetakForm.get('custPoNumber').value;
  ...
  RemoveRow(index) {
    if (index === 0) {
      ...
      var formVal = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList').value;
      var formArr = this.salesOrderBookingChetakForm.get('oeOrderLinesAllList') as FormArray;
