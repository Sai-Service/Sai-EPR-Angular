...
export class SalesOrderFormComponent implements OnInit {
  ...
  openTaxDetails(i: number) {
    ...
    if (this.taxMap.has(this.selTaxLn)) {
      ...
      // var taxValues2:any = this.taxMap.values;
      // for (let x = 0; x < this.lstgetOrderTaxDetails.length; x++) {
    ...
    // else{
    //   var lineDetailsArray = this.SalesOrderBookingForm.get('oeOrderLinesAllList') as FormArray;
    //   var x = this.SalesOrderBookingForm.get('oeOrderLinesAllList').value;
    //   this.lineTaxdetails.controls[i].push(this.fb.group({
    //     totTaxAmt: x.totTaxAmt,
    //     lineNumber: x.lineNumber,
    //     taxRateName: x.taxRateName,
    //     taxTypeName: x.taxTypeName,
        ...
        .subscribe(
          (data: any[]) => {
            ...
            for (let j = 0; j < data.length; j++) {
              ...
              this.lstgetOrderTaxDetails[j].invLineNo = i + 1;
            }
  ...
  orderLineUpdate() {
    ...
    for (let k = 0; k < orderLines.length; k++) {
      ...
      // if (orderLines[k].flowStatusCode != 'BOOKED'  || orderLines[k].flowStatusCode==='ALLOTED' || orderLines[k].flowStatusCode !='INVOICED' && orderLines[k].invType==='SS_VEHICLE' ){
      // alert('hi')
      // alert(orderLines[k].flowStatusCode +'---'+ orderLines[k].invType)
      //   if (orderLines[k].invType !='SS_VEHICLE'&& orderLines[k].flowStatusCode==='READY FOR INVOICE' && orderLines[k].flowStatusCode==='ALLOTED'){
