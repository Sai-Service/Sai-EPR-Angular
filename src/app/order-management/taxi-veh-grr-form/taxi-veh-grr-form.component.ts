...
@Component({
  selector: 'app-taxi-veh-grr-form',
  templateUrl: './taxi-veh-grr-form.component.html',
  styleUrls: ['./taxi-veh-grr-form.component.css']
})
export class TaxiVehGrrFormComponent implements OnInit {
  ...
  lineDetailsGroup(): FormGroup {
    return this.fb.group({
      ...
      invItemId: [],
      invDescription: [],
      invCategory: [{ value: '', disabled: true }],
      uom: [{ value: '', disabled: true }],
      discLineAmt: [{ value: '0', disabled: true }],
      hsnSacCode: [{ value: '', disabled: true }],
      gstPercentage: [],
      taxCategoryName: [],
      itemType: [],
      unitPrice: [{ value: '', disabled: true }],
      orderedQty: [{ value: '', disabled: true }],
      baseAmtLineWise: [0],
      poChargeAcc: [],
      taxCategoryId: [],
      segmentName: [],
    ...
    this.service.grrTaxiPO(payload).subscribe({
      next: (res: any) => {
        ...
        if (res.code === 200) {
          ...
          this.poMasterDtoForm.patchValue({
            poHeaderId: s.poHeaderId, transId: s.transId, segment1: s.segment1,
            receiptNo:s.receiptNo,status:s.status,
            suppId: s.suppId, baseAmount: s.baseAmount, totTaxAmt: s.totTaxAmt,
            totalAmt: s.totalAmt, billToLoc: s.billToLoc, shipToLoc: s.shipToLoc,
          });
          if (s.status === 'GRR') {
