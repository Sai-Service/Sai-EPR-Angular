...
@Component({
  selector: 'app-paint-purchase-order',
  templateUrl: './paint-purchase-order.component.html',
  styleUrls: ['./paint-purchase-order.component.css']
})
export class PaintPurchaseOrderComponent implements OnInit {
  paintPoForm: FormGroup;

 // @ViewChild('paintPoForm') paintPoForm: ElementRef;
 supSelCnt: number;
 isDisabled = true;
 discLineAmt: number;
 accountLocId:number;
 ...
 constructor(private fb: FormBuilder, private router1: ActivatedRoute, private router: Router, private service: MasterService) {
   this.paintPoForm = this.fb.group({
 ...
 get f() { return this.paintPoForm.controls; }
 ...
 ngOnInit(): void {
   ...
   this.paintPoForm.patchValue({ divisionName: (sessionStorage.getItem('divisionName')) })
   ...
   this.service.DepartmentListById(this.dept)
     .subscribe(
       data => {
         ...
         // this.paintPoForm.patchValue(this.DepartmentListById.divisionName);
   ...
   this.service.getLocationSearch1(this.ouId)
     .subscribe(
       data => {
         ...
         this.paintPoForm.patchValue({shipToLoc:locCodeLosi.locId});
       }
     );
   ...
   this.service.getLocationSearch1(this.ouId)
     .subscribe(
       data => {
         ...
         console.log(locCodeLosi.locId);
         this.paintPoForm.patchValue({billToLoc:locCodeLosi.locId});
       }
     );
   ...
   this.sub = this.router1.params.subscribe(params => {
     ...
     this.paintPoForm.patchValue({ segment1: this.poNo })
       ...
       .subscribe(
         data => {
           ...
           if (data.code === 400) {
             ...
             let control3 = this.paintPoForm.get('poLines') as FormArray;
             ...
             this.paintPoForm.patchValue({ poDate: poDate2 });
             // alert(approveDate2);
             ...
             if (status === 'Inprogress') {
               ...
               var control = this.paintPoForm.get('poLines') as FormArray;
               var taxcatecontrol = this.paintPoForm.get('poLines') as FormArray;
               console.log(control);
             ...
             if (status === "APPROVED") {
               ...
               let control = this.paintPoForm.get('poLines') as FormArray;
               for (let i = 0; i <= this.lstcomments1.poLines.length - lenC; i++) {
                 var poLine: FormGroup = this.lineDetailsGroup();
 ...
 //   onChanges(): void {
 //     this.paintPoForm.valueChanges.subscribe(val => {
 //
 //   });
 // }
 ...
 get lineDetailsArray() {
   ...
   return <FormArray>this.paintPoForm.get('poLines')
 }
 ...
 addRow(index) {
   ...
   var patch = this.paintPoForm.get('poLines') as FormArray;
   (patch.controls[index]).patchValue({polineNum: aa,});
...
// debugger
   ...
   var formArr = this.paintPoForm.get('poLines') as FormArray;
   var formVal = this.paintPoForm.get('poLines').value;
 ...
 updateTotAmtPerline() {
   ...
   this.paintPoForm.patchValue({ 'baseAmount': basicAmt });
   disAmt = Math.round(((disAmt) + Number.EPSILON) * 100) / 100;
   //this.paintPoForm.patchValue({ 'discAmt': disAmt });
   taxAmt1 = Math.round(((taxAmt1) + Number.EPSILON) * 100) / 100;
   this.paintPoForm.patchValue({ 'totTaxAmt': taxAmt1 });
   totAmt = Math.round(((totAmt) + Number.EPSILON) * 100) / 100;
   this.paintPoForm.patchValue({ 'totalAmt': totAmt });
 ...
 onOptionTaxCatSelected(i,event) {
   ...
   // var tacCategoryControl = this.paintPoForm.get('poLines').value;
   var tacCategoryControlNew = this.paintPoForm.get('poLines') as FormArray;
   ...
   // var taxCategoryName=this.paintPoForm.get('taxCategoryName').value;
   // alert('******'+ i+'----'+ taxCategoryName);
   if (taxCategoryName != null) {
     // var taxCategoryId=this.paintPoForm.get('taxCategoryId').value;
     var patch = this.paintPoForm.get('poLines') as FormArray;
     ...
     // var itemId=this.paintPoForm.get('itemId').value;
 ...
 onOptioninvItemIdSelected_Test(x,index) {
  ...
  alert (index + " , "+ itemCode);
   ...
   if (itemCode != null) {
    ...
    this.service.getItemDetailsByCodeNew(itemCode)
    .subscribe(
      data => {
        if(data.code ==400) { alert (data.obj +" - "+data.message);return;}

        this.itemPaint = data.obj;
        console.log(this.itemPaint);
        // patch.controls[index].patchValue({invItemId: this.itemPaint.itemId});
        var lineItemid2 = this.itemPaint.itemId;
       ...
       if (this.itemType === "GOODS") {
         this.service.ItemDetailsList(lineItemid2, this.selSuppTaxCatNm, this.billToLoc).subscribe((res: any) => {
           if (res.code === 200) {
             if (res.obj.itemId === null) {
               alert('Item Or PO Charge Account Not Found in Master !')
               var patch = this.paintPoForm.get('poLines') as FormArray;
               ...
               var gstPercentage = this.ItemDetailsList.gstPercentage;
               // alert(gstPercentage)
               // var supplierSiteId = this.paintPoForm.get('supplierSiteId').value;
               // console.log(this.suppIdList);
               // let selectedValue = this.suppIdList.find(v => v.suppSiteId == supplierSiteId);
               // console.log(selectedValue);
 ...
 fnCancatination(index) {
   ...
   arrayControl[index].segmentName = this.paintPoForm.get('segment11').value + '.'
     + this.paintPoForm.get('segment2').value + '.'
     + this.paintPoForm.get('segment3').value + '.'
     + this.paintPoForm.get('segment4').value + '.'
     + this.paintPoForm.get('segment5').value;
   // + this.paintPoForm.get('segment6').value;
   //  + this.paintPoForm.get('segment7').value + '.'
   //  + this.paintPoForm.get('segment8').value + '.'
   //  + this.paintPoForm.get('segment9').value  ;
   ...
   this.paintPoForm.get('segment11').reset();
   this.paintPoForm.get('segment2').reset();
   this.paintPoForm.get('segment3').reset();
   this.paintPoForm.get('segment4').reset();
   this.paintPoForm.get('segment5').reset();
   // this.paintPoForm.get('segment6').reset();
   this.paintPoForm.get('lookupValueDesc1').reset();
   this.paintPoForm.get('lookupValueDesc2').reset();
   this.paintPoForm.get('lookupValueDesc3').reset();
   this.paintPoForm.get('lookupValueDesc4').reset();
   this.paintPoForm.get('lookupValueDesc5').reset();
 }
 ...
 onKey(index,event) {
  ...
  // ----------------------------
   ...
   if (baseAmount != null) {
     ...
     console.log((this.paintPoForm.controls['poLines'][index]));
 ...
 UpdatePOMast() {
   this.supplierCode = this.paintPoForm.get('supplierCode').value

   // const formValue: IpostPO = this.transUpdateData(this.paintPoForm.value);
   const formValue: IpostPO = this.transUpdateData(this.paintPoForm.getRawValue());
   ...
   // var arrayControl = this.paintPoForm.get('poLines').value;
   var arrayControl1 = this.paintPoForm.get('poLines') as FormArray;
 ...
 addDiscount(i) {
   // alert(i);
   // alert('hi')
   const formValue: IpostPO = this.paintPoForm.value;
   ...
   var baseAmount = arrayControl[this.poLineTax].baseAmtLineWise;
   // alert(diss1);
   var discAmtcontrol = this.paintPoForm.get('poLines') as FormArray;
 ...
 onOptioninvitemTypeSelected(e: any, lineNum) {
   ...
   if (this.paintPoForm.get('supplierCode').value === '' || this.paintPoForm.get('shipToLoc').value === null
     || this.paintPoForm.get('suppInvNo').value === null || this.paintPoForm.get('suppInvDate').value === null
     || this.paintPoForm.get('billToLoc').value === undefined || this.paintPoForm.get('suppInvNo').value === undefined
     || this.paintPoForm.get('suppInvDate').value === undefined) {
     alert('Please Select Header Deatils !');
     this.lineDetailsArray.controls[lineNum].get('itemType').setValue('--Select--');
     (<any>this.paintPoForm.get('supplierCode')).nativeElement.focus();
     return;
   }
   else {
     if (itemType === 'GOODS') {
       var deptName1 = this.paintPoForm.get('dept').value;
       this.lineDetailsArray.controls[lineNum].get('segment').disable();
       ...
       this.paintPoForm.get('supplierCode').disable();
       this.paintPoForm.get('supplierSiteId').disable();
       this.paintPoForm.get('shipToLoc').disable();
       this.paintPoForm.get('billToLoc').disable();
 ...
 Apply() {
   const formValue: IpostPO = this.transData(this.paintPoForm.value);
 ...
 clearFormArray() {
   ...
   // this.paintPoForm.lineDetailsGroup.TaxDetailsArray.clear();
 ...
 onOptionsSelectedBranch(segment: any, lType: string) {
   ...
   this.service.getInterBranch(segment, lType).subscribe(
     data => {
       ...
       if (this.branch != null) {
         // this.paintPoForm.patchValue(this.branch);
 ...
 validateNum(index, j) {
   ...
   // this.paintPoForm.get('poLines').value
   ...
   if (value.charAt(0) === '-') {
     ...
     // this.paintPoForm.controls['poLines'].controls[index].controls['taxAmounts'].controls[j].controls.totTaxPer.value
