...
@Component({
  selector: 'app-paint-po-receipt',
  templateUrl: './paint-po-receipt.component.html',
  styleUrls: ['./paint-po-receipt.component.css']
})
export class PaintPoReceiptComponent implements OnInit {
  paintPoReceiptForm: FormGroup;
...
;
  ...
  constructor(private fb: FormBuilder, private location: Location, private router: Router, private service: MasterService, private router1: ActivatedRoute,private router3: ActivatedRoute) {
    this.paintPoReceiptForm = fb.group({
      ouName: [''],
      accountLocId: [''],
  ...
  get lineDetailsArray() {
    return <FormArray>this.paintPoReceiptForm.get('poLines')
  }
  ...
  selectAll(e) {
    ...
    let control = this.paintPoReceiptForm.get('poLines') as FormArray;
  ...
  get f() { return this.paintPoReceiptForm.controls; }

  paintPoReceipt(paintPoReceiptForm: any) {
  ...
  ngOnInit(): void {
    ...
    this.paintPoReceiptForm.patchValue({ recDate: recdate1 });
    ...
    // this.paintPoReceiptForm.patchValue(this.lstcomments1.user);
    ...
    this.sub1 = this.router1.queryParams.subscribe(params => {
      ...
      this.paintPoReceiptForm.patchValue({ shipmentNumber: this.shipmentNumber })
    ...
    this.sub = this.router1.params.subscribe(params => {
      ...
      console.log(this.paintPoReceiptForm.value);
          ...
          data => {
            ...
            if (data.code === 200) {
              ...
              if (this.lstcompolines.poStatus === 'FULLY RECEIVED') {
                ...
                this.paintPoReceiptForm.patchValue(this.lstcompolines);
                let controlinv = this.paintPoReceiptForm.get('poLines') as FormArray;
      ...
      .subscribe(
        data => {
          ...
          if (data.code === 200) {
            ...
            this.paintPoReceiptForm.patchValue(this.lstSupLineDetails);
  ...
  ReceiptFind(segment1) {
    // alert(segment1)
    this.lineDetailsArray.clear();
    // this.paintPoReceiptForm.get('loginArray').reset();
    // this.paintPoReceiptForm.get('ouName').reset();
    this.paintPoReceiptForm.get('segment1').reset();
    // this.paintPoReceiptForm.get('poType').reset();
    this.paintPoReceiptForm.get('baseAmount').reset();
    this.paintPoReceiptForm.get('supplierName').reset();
    this.paintPoReceiptForm.get('receiptNo').reset();
    this.paintPoReceiptForm.get('recDate').reset();
    this.paintPoReceiptForm.get('taxAmt').reset();
    // this.paintPoReceiptForm.get('loginName').reset();
    this.paintPoReceiptForm.get('gstDocNo').reset();
    this.paintPoReceiptForm.get('docDate').reset();
    this.paintPoReceiptForm.get('totalAmt').reset();
    this.paintPoReceiptForm.get('ewayBillNo').reset();
    this.paintPoReceiptForm.get('ewayBillDate').reset();
    this.paintPoReceiptForm.get('suppInvDate').reset();
    this.paintPoReceiptForm.get('suppInvNo').reset();
    this.paintPoReceiptForm.get('poInvNum').reset();
    this.paintPoReceiptForm.get('poInvDate').reset();
    this.paintPoReceiptForm.get('Comments').reset();
    this.displaySaveButton = false;
        ...
        .subscribe(
          data => {
            if (data.code === 200) {
              ...
              this.paintPoReceiptForm.patchValue(({ recDate: (data.obj.recDate, 'dd-MM-yyyy') }));
              let control = this.paintPoReceiptForm.get('poLines') as FormArray;
              ...
              this.paintPoReceiptForm.get('poLines').patchValue(this.lstcompolines.rcvLines);
              this.paintPoReceiptForm.patchValue(this.lstcompolines);
              this.locatorDesc = this.lstcompolines.rcvLines[0].locatorDesc;
              this.recDate = this.lstcompolines.receiptDate;

              this.paintPoReceiptForm.patchValue({ taxAmt: this.lstcompolines.totalTax });
              let controlinv1 = this.paintPoReceiptForm.get('poLines') as FormArray;
  ...
  taxDeatils(i, poHeaderId) {
    var invLine = this.paintPoReceiptForm.get('poLines').value;
  ...
  PODateWise() {
    this.displaySaveButton = false;
    var frmDate = this.paintPoReceiptForm.get('frmDate').value;
    // alert(frmDate);
    const formValue: IPODateWise = (this.paintPoReceiptForm.value);
  ...
  ReceiptDateWiseFind() {
    this.displaySaveButton = false;
    var frmDate = this.paintPoReceiptForm.get('frmDate1').value;
    // alert(frmDate);
    const formValue: IPODateWise = (this.paintPoReceiptForm.value);
    formValue.toDate = frmDate;
    formValue.billToLocId = this.locId;
  ...
  okLocator(i) {
    ...
    var poControls = this.paintPoReceiptForm.get('poLines').value;
    // var subinvetoryId = 
    // alert(this.lstcompolines.poLines[0].subInventoryId);
    poControls[i].locatorDesc =
      this.paintPoReceiptForm.get('segment11').value + '.' +
      this.paintPoReceiptForm.get('segment2').value + '.' +
      this.paintPoReceiptForm.get('segment3').value + '.' +
      this.paintPoReceiptForm.get('segment4').value + '.' +
      this.paintPoReceiptForm.get('segment5').value;
    ...
    this.service.getLocatorPoLines(locatorDesc, this.locId, this.lstcompolines.poLines[0].subInventoryId)
      .subscribe((res: any) => {
        if (res.code === 200) {
          alert(res.message);
          var patch = this.paintPoReceiptForm.get('poLines') as FormArray;
  ...
  calculation(i, qty) {
    ...
    //  alert("Validate");
    var trxLnArr = this.paintPoReceiptForm.get('poLines').value;
    var trxLnArr1 = this.paintPoReceiptForm.get('poLines') as FormArray
    let toBeIssuequantity = this.lstcompolines.poLines[i].qtyReceived;
    // var receivedqty=this.paintPoReceiptForm.get('poLines').value.qtyReceived;
  ...
  poSave() {
    var loctorDesc1 = this.paintPoReceiptForm.get('poLines').value;
    ...
    this.displaySaveButton = false;
    const totlCalControls = this.paintPoReceiptForm.get('poLines').value;
    const formValue: IpoReceipt = this.paintPoReceiptForm.value;
    formValue.baseAmount = this.paintPoReceiptForm.get('baseAmount').value;
  ...
  onOptionsLocationSelected(event) {
    // alert(event);
    this.paintPoReceiptForm.patchValue({ locId: event })
  }
  ...
  getMessage(msgType: string) {
    ...
    if (msgType.includes("Save")) {
      ...
      if (this.paintPoReceiptForm.invalid) {
