...
@Component({
  selector: 'app-paint-avg-cost',
  templateUrl: './paint-avg-cost.component.html',
  styleUrls: ['./paint-avg-cost.component.css']
})

export class PaintAvgCostComponent implements OnInit {
  pntAvgCostForm : FormGroup;
  ...
  constructor(private service: MasterService, private fb: FormBuilder, private router: Router,private location1: Location)
   ...
   lineDetailsArray() :FormArray{
      return <FormArray>this.pntAvgCostForm .get('priceListDetailList')
    }


     get f() { return this.pntAvgCostForm .controls; }
     pntAvgCost(pntAvgCostForm:any) {  }
  ...
  onOptionsSelectedStatus(event: any) {
    this.Status1 = this.pntAvgCostForm .get('status').value;
    // alert(this.Status1);
    if (this.Status1 === 'Inactive') {
      ...
      this.pntAvgCostForm .get('endDate').reset();
  ...
  transeData(val) {
   ...
   return val;
    ...
    newMast1() {
      ...
      const formValue: IAvgCostUpdate =this.transeData(this.pntAvgCostForm .value);
      this.service.AvgCostUpdateSubmit(formValue).subscribe((res: any) => {
        if (res.code === 200) {
          alert('RECORD INSERTED SUCCESSFUILY');
          this.pntAvgCostForm .reset();
        } else {
          if (res.code === 400) {
    ...
    newMast() {
          ...
          var prcLineArr = this.pntAvgCostForm.get('priceListDetailList').value;
          ...
          const formValue: IAvgCostUpdate = this.pntAvgCostForm.value;

          let variants = <FormArray>this.lineDetailsArray();

          var locationId = this.pntAvgCostForm.get('locId').value;
          var transSourceTypeId = this.pntAvgCostForm.get('transSourceTypeId').value;
          var codeCombinationId = this.pntAvgCostForm.get('codeCombinationId').value;
          var segmentName = this.pntAvgCostForm.get('segmentName').value;
          var reasonCode = this.pntAvgCostForm.get('reasonCode').value;
          var lastUpdatedBy = this.pntAvgCostForm.get('emplId').value;
          var createdBy = this.pntAvgCostForm.get('emplId').value;
  ...
  Select(priceListHeaderId: number) {
    ...
    if (select) {
      ...
      var control = this.pntAvgCostForm .get('priceListDetailList') as FormArray;
      ...
      // }
      // this.priceListMasterForm.get('priceListDetailList').patchValue(select.priceListDetailList);
      this.pntAvgCostForm .patchValue(select);
    }
  }
  ...
  onItemSelected(itemId: any)
  {
    ...
    if(itemId>0)
    {
      ...
      this.service.itemNameList(itemId)
        .subscribe(
          data => {
            ...
            this.pntAvgCostForm .patchValue(this.itemNameList.description);
      ...
      this.pntAvgCostForm .get("description").reset();
    }
...
// ======================================================================
  ...
  findByCode(searchBy,searchValue){
    ...
    var priceListHeaderId= this.pntAvgCostForm .get('priceListHeaderId').value;
    ...
    this.pntAvgCostForm .get('priceListDetailList').patchValue(data);
  }
  ...
  validateAvgPrice(index: any){
    ...
    var priceLineArr = this.pntAvgCostForm.get('priceListDetailList').value;
    ...
    if (linePrice <=0 )
    {
       ...
       var patch = this.pntAvgCostForm.get('priceListDetailList') as FormArray;
  ...
  CheckLineValidations(i) {
    ...
    var pkgLineArr = this.pntAvgCostForm.get('priceListDetailList').value;
