...
export class JobCardComponent implements OnInit {
  ...
  serchByitemIdNew1(event, i) {
   
    // var patch   = this.jobcardForm.get('jobCardLabLines') as FormArray;
    var serModel=this.jobcardForm.get('serviceModel').value;
    alert(serModel+","+event);
    let select = this.LaborItemList.find(d => d.segment = event);
    console.log(select);
    alert ("Select :" +select.description + ","+serModel);
  }


  serchByitemIdNew(event, i) {
  ...
  // alert (event.length);
    ...
    // let select = this.LaborItemList.find(d => d.segment === event.target.value);
    let select = this.LaborItemList.find(d => d.segment === event);
    // alert(select);
    // alert ("select.itemid : "+select.itemId);
    // return;
    console.log(select);
    ...
    this.serviceService.getJonCardNoSearch(jcNum)
      .subscribe(
        data => {
          ...
          var len2 = this.splitDetailsArray().length;
          ...
          // for (let j = 0; j < this.lstcomments.jobCardLabLines[0].splitArr.length ; j++) {
...
getInvItemId(event)
{
  ...
  // alert(event.target.value)
   ...
   this.userList2=[];
   if (userId.length > 2) {
    if (event.target.value.timeStamp - this.lastkeydown1 > 200) {
...
getMessage(msgType:string){
  ...
  // alert ("Testing..."+this.jobcardForm.get('regNo1').value);
      ...
      getItem(itemSeg, i){
        // alert ("Segment :" +itemSeg + " index :"+index);
        // this.serchByitemId(itemSeg,index)

        var patch = this.jobcardForm.get('jobCardLabLines') as FormArray;
        var serModel=this.jobcardForm.get('serviceModel').value;
        let select = this.LaborItemList.find(d => d.segment === itemSeg);
         if(select) {
